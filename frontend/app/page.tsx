    ...
    <div className="reference-mosaic">
      <div className="reference-main">
        ...
        <div className="reference-chart-row">
          <section className="panel reference-card"><ChartHeading eyebrow="Portfolio" title="Risk distribution" detail="Current assessed mix" /><div className="reference-donut-layout"><div className="donut-chart compact-donut"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={[{ name: 'High', value: summary.high_risk_count }, { name: 'Medium', value: summary.medium_risk_count }, { name: 'Low', value: summary.low_risk_count }]} dataKey="value" innerRadius="64%" outerRadius="88%" paddingAngle={4} stroke="none" animationDuration={900}>{['high', 'medium', 'low'].map((key) => <Cell key={key} fill={COLORS[key as keyof typeof COLORS]} />)}</Pie><Tooltip content={<RiskTooltip />} /></PieChart></ResponsiveContainer><div className="donut-center"><strong>{summary.total_customers}</strong><span>customers</span></div></div><div className="legend-list">{[['high', summary.high_risk_count], ['medium', summary.medium_risk_count], ['low', summary.low_risk_count]].map(([key, value]) => <div className="legend-row" key={String(key)}><span className={`legend-dot ${String(key)}`} /><span>{String(key)}</span><strong>{String(value)}</strong></div>)}</div></div></section>
          <section className="panel reference-card trend-card"><ChartHeading eyebrow="Momentum" title="Churn risk trend" detail="Average probability by day" />{summary.daily_trend.length ? <ResponsiveContainer width="100%" height={205}><AreaChart data={summary.daily_trend} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}><defs><linearGradient id="referenceRiskGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#b8e84b" stopOpacity={.35} /><stop offset="100%" stopColor="#b8e84b" stopOpacity={0} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#e8ede5" strokeDasharray="3 5" /><XAxis dataKey="date" tick={{ fill: '#879187', fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis domain={[0, 1]} tickFormatter={(value) => `${value * 100}%`} tick={{ fill: '#879187', fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip content={<TrendTooltip />} /><Area type="monotone" dataKey="avg_probability" stroke="#88af2a" strokeWidth={2.5} fill="url(#referenceRiskGradient)" animationDuration={900} /></AreaChart></ResponsiveContainer> : <div className="chart-empty">Trend data will appear after assessments.</div>}</section>
        </div>
